...
function itemList(userItems) {
  ...
  const showItems = (items) => {
    ...
    //create li elements
    items.forEach((item) => {
      const liElement = document.createElement('li')            //สร้าง li
      liElement.textContent = `ID:${item.id}, NAME:${item.name}, KEYWORDS:${item.keywords}` //กลาง li
      ulElement.appendChild(liElement)   //เอาไปใส่ใน ui
    })
  }
